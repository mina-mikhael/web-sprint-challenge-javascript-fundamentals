...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀:
// internal function can access the variable internal because of closure, if the internal function has no variable created with the same name it can reach out to the external function scope and work with that variable, the opposite is not possible though, external function can't work with variables declared in an internal function scope
...
function animalNames(array) {
  ...
  array.forEach((element) => {
    Arr1.push(element.animal_name);
    Arr2.push(element.scientific_name);
  });
  for (let i = 0; i < array.length; i++) {
    displayNames.push(`name: ${Arr1[i]}, scientific: ${Arr2[i]}`);
  }
  return displayNames;
}
...
function lowPopulationAnimals(array) {
  const contArr = array.filter((element) => {
...
function USApop(array) {
  const totalAnimals = array.reduce((total, currentVal) => {
...
// console.log("callbk step-1 ", consume("Mina", "Mikhael", greeting));
